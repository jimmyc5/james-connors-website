        ...
        <div className={`${reverse ? "flex-row" : "flex-row-reverse"} d-flex flex-wrap justify-content-center align-items-stretch mb-5`}>
                    <div className={`${replaceImageWith == null ? "col-10 col-xl-3 col-md-4" : "col-10 col-xl-5 col-md-6"} d-flex justify-content-center align-items-center`}>
                        <div className="col-12 col-lg-11">
                            ...
                            <a href={link} target="_blank"><img src={image} className={`rounded pictureBorder projectImage col-12 col-md-auto ${link!=null ? "projectImage projectImageHover" : "projectImage"}`} style={reverse ? {float:"left"} : {float:"right"}} /></a>
                            :
                            replaceImageWith
                            }
                        </div>
                    ...
                    <div className={`${replaceImageWith == null ? "col-12 col-md-8 col-xl-9" : "col-12 col-md-6 col-xl-7"} p-3 d-flex flex-column ${reverse ? "align-items-left" : "align-items-right"}`} style={{pointerEvents:'none'}}>
                        <div className="p-1 pb-0" style={reverse ? {textAlign:"right"} : {textAlign:"left"}}>
                            <a style={{pointerEvents:'auto'}} href={link} target="_blank" className={`${link != null ? "projectLink" : "projectHeaderNoLink"} d-flex align-items-center ${reverse ? "flex-row-reverse" : "flex-row"}`}><p className="projectTitle h4 mb-0 mr-2"> {title}</p> {link != null && <LinkSymbol fill="#AAABB8" stroke='#AAABB8' className="ms-2 me-2 inlineLinkImage"/>}</a> 
                        ...
                        <div className={`d-flex ${reverse ? "flex-row-reverse pe-3" : "flex-row ps-3"}`}>
                            {gitHubLink != null && <a style={{pointerEvents:'auto'}} href={gitHubLink} target="_blank" className="projectLink d-flex align-items-center me-2'"> <GitHubLogo fill="#AAABB8" stroke='#AAABB8' className="linkImage "/></a>}
                            {link != null && <a style={{pointerEvents:'auto'}} href={link} target="_blank" className=''><LinkSymbol fill="#AAABB8" stroke='#AAABB8' className=" inlineLinkImage"/></a> }
                        </div>
                    </div>
                </div>
    );
